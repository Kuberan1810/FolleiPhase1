...
const CampaignAndAnalysis = () => {
  // Engagement Data
  const engagementRaw = [
    { name: 'Thank you', value: 45, color: 'bg-[#004370]' },
    { name: 'Feedback', value: 24, color: 'bg-[#f7b681]' },
  ];

  const maxEngagementValue = Math.max(...engagementRaw.map(e => e.value));
  const engagementItems = engagementRaw.map(e => ({
    ...e,
    width: (e.value / maxEngagementValue) * 80 
  }));

  // Channel Performance Data
  const performanceDataRaw = [
    { label: 'Email', value: 58, colorName: 'bg-[#3ba972]', hex: '#3ba972' },
    { label: 'SMS', value: 18, colorName: 'bg-[#f9bd5e]', hex: '#f9bd5e' },
    { label: 'WhatsApp', value: 8, colorName: 'bg-[#f66161]', hex: '#f66161' },
  ];

  // Calculate dynamic circular segments
  const totalPerformanceValue = performanceDataRaw.reduce((acc, item) => acc + item.value, 0);
  const gapSize = 6; 
  const totalGaps = performanceDataRaw.length * gapSize;
  const availableSpace = 100 - totalGaps;
  
  let currentOffset = -2;
  const performanceSegments = performanceDataRaw.map(item => {
    const proportion = item.value / totalPerformanceValue;
          ...
          <div className="space-y-6">
            {engagementItems.map((item) => (
              <div key={item.name} className="flex items-center gap-4">
                <span className="text-[16px] text-[#222222] w-[100px] font-medium">{item.name}</span>
          ...
          <div className="flex items-center justify-between flex-1">
            ...
            <div className="relative w-32 h-32">
              <svg viewBox="0 0 36 36" className="w-full h-full transform -rotate-90">
                {performanceSegments.map((seg, idx) => (
                  <circle
                    key={idx}
                    cx="18"
                    cy="18"
