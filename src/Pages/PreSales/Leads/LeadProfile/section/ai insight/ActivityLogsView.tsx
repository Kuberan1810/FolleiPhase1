...
const ActivityLogsView: React.FC<ActivityLogsViewProps> = ({ onBack, leadName }) => {
  ...
  const stateLeadName = location.state?.leadName || location.state?.lead?.name;
  const displayName = leadName || stateLeadName;
  const handleBack = onBack || (() => navigate(-1));
  const [activeTab, setActiveTab] = useState<'all' | 'call' | 'email' | 'meeting'>('all');
    ...
    <div className="flex flex-col gap-6  pb-12" >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[24px] font-bold text-[#0B1C30] ">View Logs {displayName ? `- ${displayName}` : ''}</h1>
        </div>
