    ...
    <div className="flex flex-row gap-3 w-full" style={{ fontFamily: 'Manrope, sans-serif' }}>
      {cards.map((card) => (
        <div key={card.id} className="flex-1 BoxStyle flex flex-col">
          ...
          {/* Row 2: Label */}
          <div className="mt-3">
            <span style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '1.1px', lineHeight: '16.5px', fontFamily: 'Manrope' }}>
              {card.label}
            </span>
          </div>
          ...
          <div className="mt-1 flex items-baseline gap-2">
            <span style={{ fontSize: '32px', fontWeight: 700, lineHeight: '40px', color: '#0F172A', fontFamily: 'Manrope' }}>
              {card.number}
            </span>
            {card.sub && (
              <span style={{ fontSize: '12px', fontWeight: 500, color: card.id === 1 ? '#006A61' : '#94A3B8', lineHeight: '16px', fontFamily: 'Manrope' }}>
                {card.sub}
              </span>
            )}
          </div>
