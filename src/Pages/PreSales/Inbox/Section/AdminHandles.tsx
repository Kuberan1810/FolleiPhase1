...
const MOCK_DATA = [
    { id: '1', name: 'David Foster', email: 'david.f@gmail.com', score: 'Hot', status: 'CONVERTED' },
    { id: '2', name: 'Marcus Bennett', email: 'm.bennett@gmail.com', score: 'Cold', status: 'CONVERTED' },
    { id: '3', name: 'Sophia Miller', email: 'sophia.m@gmail.com', score: 'Cold', status: 'CONVERTED' },
    { id: '4', name: 'David Foster', email: 'david.f@gmail.com', score: 'Cold', status: 'CONVERTED' },
    { id: '5', name: 'Marcus Bennett', email: 'm.bennett@gmail.com', score: 'Cold', status: 'CONVERTED' },
    { id: '6', name: 'David Foster', email: 'david.f@gmail.com', score: 'Warm', status: 'CONVERTED' },
    { id: '7', name: 'David Foster', email: 'david.f@gmail.com', score: 'Warm', status: 'CONVERTED' },
    { id: '8', name: 'Alice Smith', email: 'alice@gmail.com', score: 'Hot', status: 'PRICING' },
    { id: '9', name: 'Bob Jones', email: 'bob@gmail.com', score: 'Cold', status: 'DOUBTED' },
    { id: '10', name: 'Charlie Brown', email: 'charlie@gmail.com', score: 'Warm', status: 'ANGRY' }
];
...
const AdminHandles: React.FC = () => {
    ...
    const [tempSortField, setTempSortField] = useState<'name' | 'score' | 'status' | null>('name');
    ...
    const handleToggleTempScore = (score: string) => {
        ...
        if (tempScores.includes(upperScore)) {
            setTempScores(tempScores.filter(s => s !== upperScore));
        } else {
            setTempScores([...tempScores, upperScore]);
        }
    };
    ...
    const statusOptions = ['Converted', 'Doubted', 'Angry', 'Pricing'];
    const scoreOptions = ['Hot', 'Warm', 'Cold'];

    const filteredData = MOCK_DATA.filter(act => {
        const matchesStatus = activeStatuses.length === 0 || activeStatuses.includes(act.status.toUpperCase());
        const matchesScore = activeScores.length === 0 || activeScores.includes(act.score.toUpperCase());
    ...
    const sortedData = [...filteredData].sort((a, b) => {
        ...
        if (sortField === 'name') {
            ...
            const getScoreValue = (score: string) => {
                if (score.toLowerCase() === 'hot') return 3;
                if (score.toLowerCase() === 'warm') return 2;
                if (score.toLowerCase() === 'cold') return 1;
                return 0;
            };
            valA = getScoreValue(a.score);
            valB = getScoreValue(b.score);
    ...
    const getScoreStyle = (score: string) => {
        switch (score.toLowerCase()) {
            case 'hot': return { bg: 'bg-[#FEE2E2]', text: 'text-[#DC2626]', icon: <Flame size={12} className="stroke-[2.5]" /> };
            case 'cold': return { bg: 'bg-[#E0F2FE]', text: 'text-[#2563EB]', icon: <Snowflake size={12} className="stroke-[2.5]" /> };
            case 'warm': return { bg: 'bg-[#FFEDD5]', text: 'text-[#D97706]', icon: <Sun size={12} className="stroke-[2.5]" /> };
            default: return { bg: 'bg-slate-100', text: 'text-slate-600', icon: null };
        }
    };

    const getStatusClass = (status: string) => {
        switch (status.toUpperCase()) {
            case 'CONVERTED': return 'bg-[#E4EDFF] text-[#222222]';
            case 'DOUBTED': return 'bg-[#FFE3C6] text-[#222222]';
            case 'ANGRY': return 'bg-[#FEE2E2] text-[#222222]';
            case 'PRICING': return 'bg-[#FFE7FC] text-[#222222]';
        ...
        <div className="font-manrope animate-in fade-in slide-in-from-bottom-4 duration-500 pb-16">
            <div className="mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-[30px] font-extrabold text-[#0F172A]">Admin Handles</h1>
                </div>
            ...
            <div className="flex items-center gap-6 mb-6 px-1 relative">
                ...
                <div className="relative" ref={filterRef}>
                    <button
                        ...
                        className="flex items-center gap-2 text-[#464555] hover:text-[#004370] font-manrope font-bold text-[12px] uppercase tracking-[1px] cursor-pointer transition-colors border-none bg-transparent"
                    >
                    ...
                    {isFilterOpen && (
                        <div className="absolute left-0 mt-3.5 w-[200px] bg-white z-50 font-manrope animate-in fade-in slide-in-from-top-2 duration-200 border border-slate-200 rounded-2xl p-4 shadow-[0_10px_25px_rgba(0,0,0,0.08)]">
                            <div className="mb-2">
                                ...
                                {isStatusExpanded && (
                                    <div className="space-y-1.5 pl-1 animate-in fade-in slide-in-from-top-1 duration-150 mb-4">
                                ...
                                {isScoreExpanded && (
                                    <div className="space-y-1.5 pl-1 animate-in fade-in slide-in-from-top-1 duration-150">
                                        {scoreOptions.map(score => (
                                            <label key={score} className="flex items-center gap-2.5 py-1 px-1 cursor-pointer select-none text-[14px] font-medium text-[#0B1C30] hover:text-slate-800 transition-colors">
                                                <input
                                                    type="checkbox"
                                                    checked={tempScores.includes(score.toUpperCase())}
                                ...
                                {tempSortField && (
                                    ...
                                    >
                                        ...
                                        <span className="text-[13px] font-medium font-manrope">
                                            ...
                                            {tempSortField === 'score' && 'Score'}
                                            {tempSortField === 'status' && 'Status'}
                                        </span>
                                    </div>
                                )}
                                        ...
                                        <div className="py-1">
                                            {!tempSortField ? (
                                                <>
                                                    ...
                                                    <div
                                                        className="px-8 py-2 text-[13px] font-medium font-manrope text-slate-600 hover:text-slate-900 hover:bg-slate-50 cursor-pointer transition-colors"
                                                        onClick={() => setTempSortField('score')}
                                                    >
                                                        Score
                                                    </div>
                                                    <div
                                                        className="px-8 py-2 text-[13px] font-medium font-manrope text-slate-600 hover:text-slate-900 hover:bg-slate-50 cursor-pointer transition-colors"
                                                        onClick={() => setTempSortField('status')}
                                                    >
                                                        Status
                                                    </div>
                                                </>
                            ...
                            <thead>
                                <tr>
                                    <th className="h-12 py-0 px-6 bg-[#F6FAFF] relative whitespace-nowrap">
                                        ...
                                        >
                                            <span className="text-[12px] font-semibold text-[#434655] uppercase tracking-[1.5px] group-hover/hdr:text-[#004370] transition-colors pl-2">Name</span>
                                            ...
                                            >
                                                {['All', ...'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')].map((letter) => (
                                                    <button
                                                        ...
                                                        className={`w-10 h-8 shrink-0 flex items-center justify-center text-[13px] font-bold transition-all duration-150 cursor-pointer border-none bg-transparent ${selectedLetter === letter
                                                            ? 'text-[#004370]'
                                    ...
                                    sortedData.map((act) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-4 px-6 align-middle whitespace-nowrap">
                                                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full ${getScoreStyle(act.score).bg} ${getScoreStyle(act.score).text}`}>
                                                    {getScoreStyle(act.score).icon}
                                                    <span className="text-[13px] font-semibold">{act.score}</span>
                                                </div>
                                            </td>
                                            <td className="py-4 px-6 align-middle whitespace-nowrap">
            ...
            {/* History Drawer from HandlesSection */}
            <div
