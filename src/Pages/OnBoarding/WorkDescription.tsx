...
const WorkDescription = () => {
  ...
  return (
    ...
    <div className="min-h-screen bg-[#F8F9FC] flex flex-col font-['Inter'] px-5 pt-5">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-3 sm:p-6 mt-10 sm:mt-0">
        <div className="BoxStyle p-5 sm:p-8 md:p-10 flex flex-col relative w-full shadow-lg border border-gray-100 min-h-0">
          <div className="relative flex justify-between items-center mb-6 sm:mb-8 h-12 sm:h-auto">
            <div className={`flex flex-col gap-1.5 ${searchOpen ? 'hidden sm:flex' : 'flex'}`}>
                  ...
                  <div className="relative w-full">
                    ...
                    {searchQuery && (
                      <button
                        ...
                        className="absolute right-12 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center rounded-full hover:bg-gray-100 text-[#64748B] transition-colors"
                      >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4 mb-6 justify-items-center font-inter overflow-y-auto premium-scroll pr-1 sm:pr-2 p-1">
            {filteredCategories.length > 0 ? (
              filteredCategories.map((category) => (
                <button
                  key={category.id}
                  onClick={() => toggleCategory(category.id)}
                  className={`BoxStyle flex flex-col items-start p-3 sm:p-4 gap-2 sm:gap-3 rounded-[12px] border transition-all duration-200 group text-left w-full min-h-[100px] sm:h-[110px] cursor-pointer ${selectedCategories.includes(category.id)
                ...
                >
                  ...
                  <span className="text-[13px] sm:text-[14px] font-semibold text-[#191C1E] font-inter leading-tight">
                    {category.label}
                  </span>
                </button>
              ))
          ...
          <div className="mt-6 sm:mt-auto flex items-center justify-end gap-3 sm:gap-4">
            <BtnCom
              title="Go Back"
              variant="outline"
              onClick={() => navigate(-1)}
