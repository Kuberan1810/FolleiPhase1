...
const LoginForm = () => {
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Desktop heading — hidden on mobile (mobile heading is in Login.tsx) */}
      <div className="hidden md:block text-center mb-10">
        <h2 className="text-[28px] font-semibold font-inter text-gray-900 mb-2 leading-none">Login</h2>
        <p className="text-[12px] font-medium font-manrope text-[#999999] leading-none">Let's Get You Started</p>
      </div>

      {/* Tab Switcher */}
      <div className="relative flex p-1 bg-[#F3F5F7] rounded-full mb-8 w-full mx-auto">
        {/* Sliding Pill */}
        <div
        ...
        />
        <button
          onClick={() => { setActiveTab("email"); setUsername(""); setPassword(""); }}
          className={`relative z-10 flex-1 h-[42px] md:h-[44px] text-[16px] md:text-[18px] font-normal font-inter rounded-full transition-colors duration-300 cursor-pointer ${activeTab === "email" ? "text-white" : "text-black hover:text-gray-700"}`}
        >
          Email id
        </button>
        <button
          onClick={() => { setActiveTab("mobile"); setUsername(""); setPassword(""); }}
          className={`relative z-10 flex-1 h-[42px] md:h-[44px] text-[16px] md:text-[18px] font-normal font-inter rounded-full transition-colors duration-300 cursor-pointer ${activeTab === "mobile" ? "text-white" : "text-black hover:text-gray-700"}`}
        >
          Mobile Number
        </button>
      </div>
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (activeTab === "mobile") {
                setUsername(val.replace(/[^0-9]/g, ""))
              } else {
                setUsername(val)
              }
            }}
            placeholder={activeTab === "email" ? "Enter your email" : "Enter mobile number"}
            className="w-full h-[48px] md:h-[50px] px-4 bg-white border border-[#B7AFAF] rounded-[10px] focus:outline-none focus:ring-2 focus:ring-[#004370]/20 focus:border-[#004370] transition-all font-inter text-[16px] text-black placeholder:text-gray-400"
          />
        </div>
        ...
        <button
          type="submit"
          className={`w-full h-[48px] md:h-[50px] rounded-[10px] font-normal font-inter text-[18px] md:text-[20px] transition-all duration-300 ${isFormValid
      ...
      <div className="flex items-center justify-center gap-4 my-8">
        <div className="flex-1 h-[0.5px] bg-[#999999]" />
      ...
      <p className="text-center text-gray-600 font-inter text-[13px] font-medium">
